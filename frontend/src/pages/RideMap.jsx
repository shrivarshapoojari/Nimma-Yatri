...
//     <div className="block visible z-[9999]">
...
//       <MapContainer center={[12.95, 77.6]} zoom={12} style={{ height: "200px", width: "100%" }}>
//         <TileLayer url="https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png" />
...
const getNumberedIcon = (number) =>
  new L.DivIcon({
    className: "custom-marker",
    html: `<div style="background-color:blue;color:white;border-radius:50%;width:20px;height:20px;
    display:flex;align-items:center;justify-content:center;font-size:12px;font-weight:bold;">${number}</div>`,
  });
...
const AnimatedRoute = ({ positions, setHoveredDirection }) => {
  ...
  useEffect(() => {
    ...
    antPath.on("mouseover", (e) => {
      const latlng = e.latlng;
      const closestPoint = positions.find(
        (pos) => pos[0] === latlng.lat && pos[1] === latlng.lng
      );
      if (closestPoint) {
        const index = positions.indexOf(closestPoint);
        setHoveredDirection(locations[index]?.direction || "");
...
const RideMap = ({ isOpen, onClose }) => {
    ...
    const fetchData = async () => {
      try {
        const response = await axios.get(
          `${import.meta.env.VITE_BASE_URL}/rides/getrides`,
          {
            headers: {
              Authorization: `Bearer ${localStorage.getItem("token")}`,
            },
  ...
  useEffect(() => {
    const fetchAllCoordinates = async () => {
      ...
      setLoc(fetchedLocations.filter((loc) => loc !== null));
    };
    fetchAllCoordinates();
  }, [ridePath]);
  ...
  return (
    <div className="fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-white shadow-xl px-8 py-4 rounded-lg max-w-[700px] z-[9999] w-3/4 h-3/4">
      {/* Close Button */}
      ...
      <div className="flex flex-col items-center mb-3">
        ...
        <span className="text-green-600 font-bold text-lg mt-1">{earning}</span>
        <span className="text-gray-600 text-sm">Expected Earnings</span>
      </div>

      {/* Map Section */}
      <MapContainer center={[12.95, 77.6]} zoom={12} style={{ height: "250px", width: "100%" }}>
        <TileLayer url="https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png" />
        <AnimatedRoute positions={loc.map((lo) => lo.coords)} setHoveredDirection={setHoveredDirection} />
        {loc.map((lo, i) => (
          <Marker key={i} position={lo.coords} icon={getNumberedIcon(i + 1)}>
      ...
      {/* Display Directions on Hover */}
      {hoveredDirection && (
        <div className="absolute top-2 left-1/2 transform -translate-x-1/2 bg-black text-white p-2 rounded text-sm">
          {hoveredDirection}
        </div>
      )}
    </div>
